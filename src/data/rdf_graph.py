...
class RDFGraph:
    def __init__(self, text, max_entail=1, top_tfidf=20000, top_n_rel=50,
        ...
        d) top_n_ent = the same interpretation as 'top_n_rel', except for
        ...
        # Define Inputs
        self.text = {indx: text for indx, text in enumerate(text)}
    ...
    def cluster_entities(self, link_method='average'):
        ...
        dist_vec = pdist(entity_list.reshape(-1, 1), lambda u, v: 1 - ratio(u[0], v[0]))
    ...
    def extract_raw_tuples(self, confidence_thres=1, coref_opt=False):
        ...
        for num, text in all_text.items():
            if num % 100 == 0:
                print('{} articles out of {} processed'
                      .format(num, len(all_text)))
            # Start information extraction
            # The output of CoreNLP can return many different fields depending
            # on how well the parsing did, so there are a lot of 'if-then' and
            # 'try-except' conditions below to catch these different
            # possibilities
            xml_parsed_doc = self.stanford_parse(text, max_entail, coref_opt)

            if xml_parsed_doc is None:
                print('article parse failed')
                all_tuples.update({num: None})
                continue
            elif 'Request is too long to be handled' in xml_parsed_doc:
                print('Article is too large for parser, need to split up')
                all_tuples.update({num: None})
                continue
            elif 'CoreNLP request timed out' in xml_parsed_doc:
                print('article parse failed')
                all_tuples.update({num: None})
                continue

            text_tuples = self.stanford_parse_reader(xml_parsed_doc, confidence_thres)
            all_tuples.update({num: text_tuples})
        self.all_tuples = all_tuples

        return all_tuples

    @staticmethod
    def stanford_parse(text, max_entail, coref_opt):
        # Parse text using stanford CoreNLP, with option of coreference resolution
        if coref_opt:
            try:
                xml_parsed_doc = nlp.annotate(text,
                                              properties={
                                                  "annotators": "tokenize,ssplit,pos,depparse,natlog,openie,dcoref",
                                                  'timeout': '1000000',
                                                  "outputFormat": "xml",
                                                  "openie.triple.strict": "true",
                                                  "openie.max_entailments_per_clause": str(max_entail),
                                                  "opeie.openie.resolve_coref": "true",
                                              }
                                              )
            ...
            try:
                ...
                return None
        return xml_parsed_doc

    @staticmethod
    def stanford_parse_reader(parsed_text, confidence_thres):
        # Function to read out complicated xml structure output from CoreNLP
        text_tuples = []
        try:
            json_parsed_doc = xmltodict.parse(parsed_text)
        # sometimes the parser returns something really weird, using an
        # overly broad except to catch it for now
        except:
            print('article parse failed')
            return None
